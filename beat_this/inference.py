...
def aggregate_prediction(
    ...
    Args:
        pred_chunks (list): List of prediction chunks, where each chunk is a dictionary containing 'boundary' predictions.
    ...
    Returns:
        torch.Tensor: The aggregated boundary predictions as a torch tensor for the whole piece.
    """
    if border_size > 0:
    ...
    piece_prediction_boundary = torch.full((full_size,), -1000.0, device=device)
    ...
    for start, pchunk in zip(starts, pred_chunks):
        piece_prediction_boundary[
            start + border_size : start + chunk_size - border_size
        ] = pchunk["boundary"]
    return piece_prediction_boundary
...
def split_predict_aggregate(
    ...
    Returns:
        dict: the model framewise predictions for the whole piece as a dictionary containing 'boundary' predictions.
    """
    ...
    # remove the extra dimension in boundary prediction due to batch size 1
    pred_chunks = [
        {"boundary": p["boundary"][0]} for p in pred_chunks
    ]
...
class Audio2Frames(Spect2Frames):
    """
    Class for extracting framewise phrase boundary predictions (logits) from an audio tensor.
    """
...
class Audio2Boundaries(Audio2Frames):
    """
    Class for extracting phrase boundary positions (in seconds) from an audio tensor.

    Args:
        checkpoint_path (str): Path to the model checkpoint file. It can be a local path, a URL, or a key from the CHECKPOINT_URL dictionary. Default is "final0", which will load the model trained on harmonixset.
        ...
        dbn (bool): Not applicable for phrase boundaries. Default is False.
    """
    ...
    def __call__(self, signal, sr):
        ...
        return self.frames2boundaries(boundary_logits)
