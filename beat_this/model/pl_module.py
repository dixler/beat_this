...
class PLBeatThis(LightningModule):
    ...
    def _compute_loss(self, batch, model_prediction):
        ...
        boundary_loss = self.boundary_loss(
            model_prediction["boundary"], batch["truth_boundary"].float(), boundary_mask
        )
        # return loss in a dictionary for logging
        return {
    ...
    def _compute_metrics(self, batch, postp_boundary, step="val"):
        """ """
        # compute for boundary
        metrics_boundary = self._compute_metrics_target(
            batch, postp_boundary, target="boundary", step=step
        )

        return metrics_boundary
...
class Metrics:
    def __init__(self, eval_trim_beats: int) -> None:
        self.min_boundary_time = eval_trim_beats

    def __call__(self, truth, preds, step) -> Any:
        # Trim boundaries at the start of the track
        truth = truth[truth >= self.min_boundary_time]
        preds = preds[preds >= self.min_boundary_time]
        
        if (
            step == "val"
        ):  # limit the metrics that are computed during validation to speed up training
            # Use segment boundary evaluation metrics
            precision, recall, fmeasure = mir_eval.segment.detection(
                truth, preds, window=0.5, trim=False
            )
            return {"F-measure": fmeasure, "Precision": precision, "Recall": recall}
        elif step == "test":  # compute all metrics during testing
            # Compute metrics with different tolerances
            precision_05, recall_05, fmeasure_05 = mir_eval.segment.detection(
                truth, preds, window=0.5, trim=False
            )
            precision_3, recall_3, fmeasure_3 = mir_eval.segment.detection(
                truth, preds, window=3.0, trim=False
            )
            return {
                "F-measure_0.5s": fmeasure_05,
                "Precision_0.5s": precision_05,
                "Recall_0.5s": recall_05,
                "F-measure_3s": fmeasure_3,
                "Precision_3s": precision_3,
                "Recall_3s": recall_3,
            }
        else:
